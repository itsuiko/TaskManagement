...
**削除ボタンは左端に離して置く。** 保存・キャンセルと並べると、押すつもりのないものを押しやすい。**削除は取り消せない**ので距離を取る。

押すと編集モーダルを閉じてから S-04 に移る。2枚を重ねて開かないのは、上を閉じたときに下が残り、**消したはずのカードの編集画面**が出てくるためである。
...
**F-04「誤操作を防ぐため、削除前に確認を求める」。** 削除は取り消せない——行ごと消える物理削除で、元に戻す手段を持っていない（[データ設計書](./data-design.md) 7章）。**この確認の一手が、取り消し機能の代わりになっている。**

**何を消すのかをタイトルで見せる。** 「本当に削除しますか？」だけだと、どのカードを開いていたか分からなくなったときに確かめようがない。

**失敗したときは閉じない。** 閉じるとボードに戻ってしまい、消えたのか消えていないのかを利用者が判断できなくなる（S-02 / S-03 と同じ）。
...
| 4.2 | 2026-09-12 | **S-03 に「削除」ボタンを追加し、S-04 削除確認ダイアログの説明を書いた。** 版4.1 で「API がまだ無いので併設していない」としていたものを実装済みに変更。削除ボタンを左端に離す理由（取り消せない操作なので保存・キャンセルから距離を取る）と、編集モーダルを閉じてから S-04 を開く理由（`<dialog>` を重ねると、上を閉じたときに消したはずのカードの編集画面が残る）を記載 |
